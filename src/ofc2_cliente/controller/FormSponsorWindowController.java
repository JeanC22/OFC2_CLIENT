...
public class FormSponsorWindowController{
    ...
    private void btnCreateSponsor(ActionEvent event) {
        try {
            ...
            Integer phone = Integer.parseInt(txtFPhone.getText());
            Boolean status = chbxState.isSelected();
            Sponsor s = new Sponsor(name, phone, email, status);
            if (!sponsorList.contains(s)) {
                this.sp = s;
                try {
                    Stage mainStage = new Stage();
                    URL viewLink = getClass().getResource(
    ...
    public void lista(ObservableList<Sponsor> sponsorList){
